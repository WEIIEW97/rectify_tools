...
cv::Mat apply_distortion(cv::Mat x, cv::Mat k) {
  ...
  for (i = 0; i < cols; i++) {
    ...
    a2.at<double>(0, i) = r2.at<double>(0, i) + 2 * pow(x.at<double>(0, i), 2);
    a3.at<double>(0, i) = r2.at<double>(0, i) + 2 * pow(x.at<double>(1, i), 2);
  }
...
lutVecName bilinear2x2(int coordtype, bool reverseMapping, cv::Mat xMatSampled,
  ...
  for (row = 0; row < lut.rows; row++) {
    for (col = 0; col < lut.cols; col++) {
      lut.ptr<double>(row)[col] =
          cvRound(pow(2, 5) * lut.ptr<double>(row)[col]) / (double)pow(2, 5);
    }
  }
  ...
  for (size_t i = 0; i < lutVecN.rows; i++) {
    if (lutVecN.ptr<double>(i)[0] >= 0) {
      ...
      if (i < 8) {
        bin_x = dec2bin(pow(2, t - 1) + lutVecN.ptr<double>(i)[0], t - 1);
      } else if (i >= 8 && i < step1) {
...
lutXY gen_lut(initParams* params, cv::Mat intrOld, cv::Mat kc, cv::Mat intrNew,
  ...
  double lutSize[2] = {
      static_cast<double>(std::max(sampleY.rows, sampleY.cols)),
    ...
    for (size_t i = 0; i < validMat.rows; i++) {
      for (size_t j = 0; j < validMat.cols; j++) {
        if (validMat.ptr<double>(i)[j] == 0) {
          ...
          inValidLutX.ptr<double>(i)[j] = deltaLutX2.ptr<double>(i)[j];
          inValidLutY.ptr<double>(i)[j] = deltaLutY2.ptr<double>(i)[j];
        }
      }
    }
